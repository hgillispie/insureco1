...
const STEPS = [
  ...
  { label: 'Coverage Type', key: 'coverage-type' },
      ...
      <div className="insurance-options">
        {INSURANCE_OPTIONS.map(option => (
          <button
            key={option.id}
            type="button"
            className={`insurance-tile ${formData.insuranceType === option.id ? 'insurance-tile--selected' : ''}`}
            onClick={() => onChange('insuranceType', option.id)}
            aria-pressed={formData.insuranceType === option.id}
          >
            <div className="insurance-tile__icons">
              {option.icons.includes('car') && <CarIcon />}
              {option.icons.includes('home') && <HomeIcon />}
            </div>
...
function ReviewStep({ formData }) {
  ...
  return (
    <div className="signup-step__fields signup-review">
    ...
    <div className="signup-page">
      ...
      <div className="signup-form-card">
        <div className="signup-form-card__header">
          <h2 className="signup-form-card__title">{stepMeta.title}</h2>
          <p className="signup-form-card__description">{stepMeta.description}</p>
        </div>

        <div className="signup-form-card__body">
          {currentStep === 0 && (
            <PersonalInfoStep formData={formData} onChange={handleChange} />
          )}
          {currentStep === 1 && (
            <AddressStep formData={formData} onChange={handleChange} />
          )}
          {currentStep === 2 && (
            <InsuranceTypeStep formData={formData} onChange={handleChange} />
          )}
          {currentStep === 3 && (
